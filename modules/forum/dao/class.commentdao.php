...
Class CommentDAO{
	...
	public function getAllComments(){
		...
		for($i = 0; $i < sizeof($rows); $i++) {
		    ...
		    $this->_Comment->setDiscussionID( $row['DiscussionID'] );
				...
				$this->_Comment->setCommentVotes( $row['CommentVotes']);
	...
	public function readDiscussion($Discussion){
		...
		$this->_Discussion->setVotes( $row['Votes']);
	...
	public function readComment($Discussion){
		...
		$this->_DB->doQuery("SELECT * FROM tbl_discussion_comment WHERE DiscussionID = '$Discussion' ORDER BY CommentVotes DESC");
	...
	public function incrementDiscussionViews($DiscussionID) {
		$SQL = "UPDATE tbl_discussion SET tbl_discussion.Views = tbl_discussion.Views + 1 WHERE ID = '$DiscussionID'";
		$SQL = $this->_DB->doQuery($SQL);
